...
public class Controller implements AbortionChecker {
	...
	public void eventMouseClick(int x, int y, int button) throws IOException {
		...
		// write window to fasta files
			if(Config.getInstance().getIntVal("SmithWaterman", 0) == 1){
				sticking = false;
				reversestick = false;
				crossX = x;
				crossY = y;
				...
				ClientGlobals.infoMessage("Exporting current sequence window with Smith Waterman alignment to FASTA files:\n" + "Sequence 1: " + (crossPosX - 500)+ "-"
						+ (crossPosX + 500)+ "\n" + "Sequence 2: " + (crossPosY-500) + "-" + (crossPosY + 500));
				...
				String defFile = ClientGlobals.insertBeforeExtension(ClientGlobals.extractFilename(dpInfo.seqFile1),
						"_" + (crossPosX - 500) + "-" + (crossPosX+500) + "_" + ClientGlobals.extractFilename(dpInfo.seqFile2).replace(".fasta", "") + "_" + (crossPosY-500) + "-" + (crossPosY+500) + "_" + "sw");
				f.setSelectedFile(new File(defFile));
				int retval = f.showSaveDialog(mp);
				if (retval == JFileChooser.APPROVE_OPTION) {
					String file1 = f.getSelectedFile().getAbsolutePath();
					int window = 1000;
					Smithexport.export(dpInfo.sequence1, dpInfo.sequence2,crossPosX, crossPosY, file1, window, cp.reverseComplementaryAlignments(), dpInfo.submat);
				p.setCrossHair(crossX, crossY, crossPosX, crossPosY);
				dp.repaint();
				}
			...
			if(Config.getInstance().getIntVal("NeedlemanWunsch", 0) == 1){
				sticking = false;
				reversestick = false;
				crossX = x;
				crossY = y;
				...
				ClientGlobals.infoMessage("Exporting current sequence window with Needleman Wunsch alignment to FASTA files:\n" + "Sequence 1: " + (crossPosX-500) + "-"
						+ (crossPosX + 500)+ "\n" + "Sequence 2: " + (crossPosY-500) + "-" + (crossPosY + 500));
				String lastdir = Config.getInstance().getStringVal("lastopendir_seqexport", "");
				// first file
				JFileChooser f = new JFileChooser(lastdir);
				String defFile = ClientGlobals.insertBeforeExtension(ClientGlobals.extractFilename(dpInfo.seqFile1),
						"_" + (crossPosX-500) + "-" + (crossPosX+500) + "_" + ClientGlobals.extractFilename(dpInfo.seqFile2).replace(".fasta", "") + "_" + (crossPosY-500) + "-" + (crossPosY+500) + "_" + "nw");
				f.setSelectedFile(new File(defFile));
				int retval = f.showSaveDialog(mp);
				if (retval == JFileChooser.APPROVE_OPTION) {
					String file1 = f.getSelectedFile().getAbsolutePath();
					int window = 1000;
					Needleexport.export(dpInfo.sequence1, dpInfo.sequence2,crossPosX, crossPosY, file1, window, cp.reverseComplementaryAlignments(), dpInfo.submat);
				p.setCrossHair(crossX, crossY, crossPosX, crossPosY);
				dp.repaint();
				}
			...
			if (Config.getInstance().getIntVal("NeedlemanWunsch", 0) != 1 && Config.getInstance().getIntVal("SmithWaterman", 0) != 1){
				// user message
				...
				JOptionPane.showMessageDialog((Component) container, "Failed exporting: " + e.getMessage());
				e.printStackTrace();
			}
